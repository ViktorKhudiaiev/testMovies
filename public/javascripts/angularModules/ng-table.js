...
var ngTableController = ['$scope', 'ngTableParams', '$timeout', function ($scope, ngTableParams, $timeout) {
    ...
    $scope.dd = function() {
        $('.ui.dropdown').dropdown({
         onShow: function () {
            $(this).closest('table').removeClass('collapsing');
            },
            onHide: function () {
            $(this).closest('table').addClass('collapsing');
            }
        });
    }
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
    ...
    $templateCache.put('ng-table/filters/select.html', 
        '<div class="ui selection dropdown filter-dropdown" ng-mouseover="dd()" ng-model="params.filter()[name]">'+
            '<input type="hidden" name="gender" >'+
            '<div class="default text"></div>'+
            '<i class="dropdown icon"></i>'+
            '<div class="menu">'+
              '<div class="item" ng-repeat="data in column.data" ng-click="params.$params.filter[name] = (data.title != all) ? data.title : empty">{{data.title}}</div>'+
            '</div>'+
        '</div>'
//          '<select ng-options="data.id as data.title for data in column.data" ng-model="params.filter()[name]" ng-show="filter==\'select\'" class="filter filter-select form-control" name="{{column.filterName}}"> </select>'
    ...
    $templateCache.put('ng-table/header.html', '' +
            '<tr>' + 
                ...
                '</th>' +
            '</tr>' +
            '<tr ng-show="show_filter" class="hided-filter-row">' +
                '<th ng-repeat="column in $columns" ng-show="column.show(this)" class="filter hided-filter-th">' +
                    '<div ng-repeat="(name, filter) in column.filter" class="hided-filter-div">' + 
    ...
    '<div>'+
            '<div class="five wide right floated column">'+
                '<div class="ng-cloak ui right floated pagination menu"> ' +
                '       <a ng-if="params.settings().counts.length" ng-repeat="count in params.settings().counts" type="button" ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" class="item"> ' +
                '           <span ng-bind="count"></span> ' +
                '       </a> ' +
                '   </div> ' +
                '    <div class="ui right floated pagination menu">' +
                '       <a class="item" ng-class="{\'disabled\': !page.active}" ng-click="params.page(page.number); $event.stopPropagation();" ng-repeat="page in pages" ng-switch="page.type"> ' +
                '           <div ng-switch-when="prev"  href="">&laquo;</div> ' +
                '           <div ng-switch-when="first" href="">' +
                '               <span ng-bind="page.number"></span>' +
                '           </div> ' +
                '           <div ng-switch-when="page" href="">' +
                '               <span ng-bind="page.number"></span>' +
                '           </div> ' +
                '           <div ng-switch-when="more" href="">&#8230;</div> ' +
                '           <div ng-switch-when="last" href="">' +
                '               <span ng-bind="page.number"></span>' +
                '           </div> ' +
                '           <div ng-switch-when="next" href="">&raquo;</div> ' +
                '       </a> ' +
                '   </div>' +
            '</div>'+
        '</div>');
